<?php
    ...
    if(isset($_POST['submit'])){
         ...
         */
        $sqlUser = "SELECT * FROM `student` WHERE `username` = '".$username."'";
        $checkUser = $conn->query($sqlUser);
        if( !$checkUser->num_rows ) {
            $sql = "INSERT INTO `student` (`username`, `password`,`prefix`, `first_name`, `last_name`, `id_student`, `branch`, `email`, `telephone`, `created_at`) 
            VALUES ('".$username."', 
                    '".$passwordHashed."', 
                    '".$_POST['prefix']."', 
                    '".$_POST['first_name']."', 
